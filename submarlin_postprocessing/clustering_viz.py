...
def convert_seconds_to_hours(
    ...
    var_name: str = 't_idiv',
    metadata_col_name: str = 'col_name_last_t'
):
    col_name = metadata.loc[var_name, metadata_col_name]
    return df.assign(**{col_name: lambda df_: df_[col_name] / 3600})

def adjust_growth_rate_base_e_to_2(
    df,
    metadata: pd.DataFrame,
    var_name: str = 'growth_rate',
    metadata_col_name: str = 'col_name_last_t'
):
    col_name = metadata.loc[var_name, metadata_col_name]
...
## DEFINE STUFF
    ...
    return (
        ...
        .assign(
            ...
            col_name_steady_state_estimators = {
                't_idiv': 'Mean (Robust)_Delta time (s)',
            ...
            # For slope analysis
            # col_name_pval_df = {
            #     't_idiv': ('Value', 'Delta time (s)'),
            #     'growth_rate': ('Value', 'growth_rate_hr'),
            #     'intensity': ('Value', 'mCherry mean_intensity'),
            #     'length': ('Value', 'Length'),
            #     'sep_disp': ('Value', 'Septum Displacement Length Normalized'),
            #     'width': ('Value', 'Width'),
            # }
        )
    )
...
def generate_aggregated_heatmap_df(
    ...
    The resulting dataframe has one row per cluster
    '''
    return (
...
def read_and_process_clustering_df(
    clustering_df_filename,
    plot_metadata
) -> pd.DataFrame:
    '''
    Read and process the clustering DataFrame.
    '''
    return (
        pd.read_pickle(clustering_df_filename)
...
def combine_cluster_groups(
    df: pd.DataFrame,
    order_to_show: list,
    cluster_groups: dict,
    new_order_to_show: list,
    drop_indices: list = None
) -> pd.DataFrame:
    """Reorder rows, collapse specified cluster groups into mean rows, reorder again, optionally drop indices."""
    df_out = df.loc[order_to_show, :].copy()
    for cluster_group_id, cluster_numbers in cluster_groups.items():
        df_subset = df_out.loc[cluster_numbers, :]
        df_mean = df_subset.mean(axis=0).to_frame().T
        df_mean.index = [cluster_group_id]
        df_out = pd.concat([df_out.drop(index=cluster_numbers), df_mean], axis=0)
    df_out = df_out.loc[new_order_to_show, :]
    if drop_indices:
        df_out = df_out.drop(index=drop_indices, errors='ignore')
    return df_out
...
class ClusteringVisualization:
    def __init__(self, exp_group: str, categories_controls: list):
        ...
        self.clustering_df = read_and_process_clustering_df(
            self.clustering_df_filename,
            self.plot_metadata
        )
...
# df_heatmap_zscores_L3 = combine_cluster_groups(
...
# df_heatmap_summary_L3 = combine_cluster_groups(
